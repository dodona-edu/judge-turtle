...
if [ ! -f /requirements/judge-turtle.txt ]; then
    echo "The running image ships no /requirements/judge-turtle.txt, so it predates the baked dependency list and there is nothing to compare requirements.txt against. Expected until the image carrying that file is published (dodona-edu/docker-images#439, plus a publish run)." | report
    exit 1
fi
